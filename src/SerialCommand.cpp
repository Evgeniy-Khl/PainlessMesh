...
void SerialCommand::readChar(char inChar) {
  if (inChar == term) {     // Check for the terminator (default '\n') meaning end of command
    ...
    char *command = strtok_r(buffer, delim, &last);   // (первый вызов) Search for command at start of buffer
    if (command != NULL) {
    ...
    clearBuffer();
    prompt();
  } else if(inChar == 0x8 && bufPos > 0) { // возврат на шаг назад
    if(echoEnabled) {
      serial->write(0x08);
      serial->print(' ');
      serial->write(0x08);
...
char *SerialCommand::next() {
  return strtok_r(NULL, delim, &last);  // (второй вызов) возвращают указатель на следующий токен или NULL. https://ru.manpages.org/strtok_r/3
}
