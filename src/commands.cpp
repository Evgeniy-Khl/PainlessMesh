...
void unescape() {       // Выход из «экранированного» режима
...
void setRst() {
    char* state = commands.next();      // возвращают указатель на следующий токен

    if(state == NULL) {
        pinMode(UC_NRST, INPUT);
    } else {
        bool nrstState = atoi(state);   // конвертирует строку, на которую указывает параметр state, в величину типа int
...
void monitorBridge() {
    ...
    monitorEnabled = atoi(state);   // конвертирует строку, на которую указывает параметр state, в величину типа int
}
...
void connected() {
    ...
    int target = atoi(state);       // конвертирует строку, на которую указывает параметр state, в величину типа int
    if(target) {
        connectedHigh = true;
